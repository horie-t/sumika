...
interface TransactionJpaRepository
  ...
  Optional<TransactionJpaEntity> findByUserIdAndId(String userId, Long id);

  boolean existsByUserIdAndCategoryId(String userId, Long categoryId);

  long deleteByUserIdAndId(String userId, Long id);

  /** その利用者の、期間内の収支をカテゴリ別に合計する（カテゴリ名・種別はアドホック結合で取得）。 */
  @Query(
      """
  ...
  /** その利用者の、期間内の収支を年月・種別ごとに合計する（Postgres の EXTRACT を使う native クエリ）。 */
  @Query(
      value =
          """
